...
class DashboardApp {
          ...
          <tr class="${isSelected ? 'bg-amber-50/80 border-l-4 border-l-amber-600' : 'hover:bg-slate-50'} transition border-b border-slate-200">
            ...
            <td class="p-3">
              <button onclick="app.toggleKapanewon('${kId}', '${SecurityUtils.escapeHTML(a.kapanewon)}')" class="px-3.5 py-1.5 ${isSelected ? 'bg-slate-900 hover:bg-slate-800 text-amber-400 border border-slate-700' : 'bg-amber-600 hover:bg-amber-700 text-white'} rounded-lg text-xs font-bold whitespace-nowrap shadow-sm transition inline-flex items-center gap-1" aria-label="${isSelected ? 'Batalkan pilihan wilayah ' + SecurityUtils.escapeHTML(a.kapanewon) : 'Pilih wilayah ' + SecurityUtils.escapeHTML(a.kapanewon)}" title="${isSelected ? 'Klik untuk membatalkan filter' : 'Klik untuk memfilter data wilayah ini'}">
                ${isSelected ? 'Batal Pilih <svg height="14" class="flex-shrink-0 w-3.5 h-3.5 inline-block ml-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>' : 'Pilih Wilayah <svg height="14" class="flex-shrink-0 w-3.5 h-3.5 inline-block ml-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>'}
              </button>
            </td>
          </tr>
        `;
      ...
      if (match) {
        ...
        if (key === activeKey) {
          ...
          iconSpan.innerHTML = this.sortDirection === 'asc'
            ? '<svg height="14" class="flex-shrink-0 w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 19V5M5 12l7-7 7 7"/></svg>'
            : '<svg height="14" class="flex-shrink-0 w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14M19 12l-7 7-7-7"/></svg>';
        } else {
          ...
          iconSpan.innerHTML = '<svg height="14" class="flex-shrink-0 w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M7 15l5 5 5-5M7 9l5-5 5 5"/></svg>';
        }
      }
    });
  }
  ...
  updateUserUI() {
    ...
    if (this.user) {
      desktopContent = `
        <span class="text-xs text-amber-300 font-bold whitespace-nowrap mr-1 inline-flex items-center gap-1.5"><svg height="14" width="14" class="w-3.5 h-3.5 text-amber-300 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> ${SecurityUtils.escapeHTML(this.user.name)}</span>
        <a href="${relPrefix}security/index.html" class="px-2.5 py-1.5 bg-blue-900 hover:bg-blue-800 text-white border border-blue-700 rounded-lg font-bold text-xs transition inline-flex items-center gap-1.5 shadow-sm"><svg height="14" width="14" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> Audit Log</a>
      ...
      mobileContent = `
        <div class="w-full flex items-center justify-between bg-slate-800 p-2.5 rounded-lg border border-slate-700 mb-2">
          <span class="text-xs text-amber-300 font-bold"><svg height="14" width="14" class="flex-shrink-0 w-3.5 h-3.5 text-amber-300 inline-block mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> ${SecurityUtils.escapeHTML(this.user.name)}</span>
        ...
        <a href="${relPrefix}security/index.html" class="w-full text-center px-3 py-2 bg-blue-900 hover:bg-blue-800 text-white rounded-lg font-bold text-xs transition block border border-blue-700"><svg height="14" width="14" class="w-3.5 h-3.5 inline-block mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> Audit Log Keamanan</a>
      `;
    } else {
      desktopContent = `
        <a href="${relPrefix}security/index.html" class="px-3 py-1.5 bg-blue-900/90 hover:bg-blue-800 text-white border border-blue-600/80 rounded-lg font-bold text-xs transition inline-flex items-center gap-1.5 shadow-sm"><svg height="14" width="14" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> Status Keamanan</a>
        <a href="${relPrefix}login/index.html" class="px-3.5 py-1.5 bg-amber-600 hover:bg-amber-700 text-white font-extrabold rounded-lg text-xs transition shadow-sm inline-flex items-center gap-1.5"><svg height="14" width="14" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg> Masuk Pimpinan</a>
      `;
      mobileContent = `
    ...
    const modalHtml = `
      <div id="auditModalOverlay" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white border border-slate-300 rounded-2xl max-w-3xl w-full p-6 shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
          <div class="flex items-center justify-between border-b border-slate-200 pb-3">
            <div>
              <h3 class="text-sm font-black text-slate-900 uppercase tracking-wide"><svg height="14" width="14" class="w-3.5 h-3.5 inline-block mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> LOG AUDIT KEAMANAN &amp; INTEGRITAS DATA</h3>
