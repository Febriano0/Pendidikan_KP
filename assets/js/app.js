...
class DashboardApp {
  ...
  updateQuickKPIStats(kapId) {
    ...
    if (zoneBadge) {
      if (kapId === "all") {
        ...
        zoneBadge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-200";
      } else {
        const zone = foundKap ? foundKap.zone : "Wilayah";
        zoneBadge.textContent = `Zona ${zone} (${foundKap ? foundKap.id.toUpperCase() : ''})`;
        zoneBadge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-amber-100 text-amber-900 border border-amber-300";
      }
    }
    ...
    if (route === "dashboard") {
      if (kapId === "all") {
        ...
        if (s1Badge) { s1Badge.textContent = "PAUD, SD, SMP"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
        s2Val.textContent = "99.2%";
        if (s2Badge) { s2Badge.textContent = "Melek Huruf"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        s3Val.textContent = "84.6";
        if (s3Badge) { s3Badge.textContent = "SPM Tuntas"; s3Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-purple-100 text-purple-800 border border-purple-300"; }
      } else {
        ...
        if (s1Badge) { s1Badge.textContent = "Terakreditasi A/B"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
        s2Val.textContent = itemAkses ? itemAkses.apk : "78.5%";
        if (s2Badge) { s2Badge.textContent = "APK Wilayah"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        ...
        if (s3Badge) {
          ...
          s3Badge.className = isDanger ? "text-xs font-bold px-3 py-1 rounded-lg bg-red-100 text-red-800 border border-red-300" : "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300";
        }
      }
    } else if (route === "sub1") {
      if (kapId === "all") {
        s1Val.textContent = "76.4% / 97.2%";
        if (s1Badge) { s1Badge.textContent = "Rerata APK / APM"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
        s2Val.textContent = "2,280 Siswa";
        if (s2Badge) { s2Badge.textContent = "Total ATS 12 Kapanewon"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-amber-100 text-amber-800 border border-amber-300"; }
        s3Val.textContent = "12 Terpetakan";
        if (s3Badge) { s3Badge.textContent = "3 Butuh Merger SD"; s3Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-purple-100 text-purple-800 border border-purple-300"; }
      } else {
        ...
        if (s1Badge) { s1Badge.textContent = item ? `APM: ${item.apm.split('/')[0].trim()}` : "Stabil"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        ...
        if (s2Badge) {
          ...
          s2Badge.className = isHigh ? "text-xs font-bold px-3 py-1 rounded-lg bg-red-100 text-red-800 border border-red-300" : "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300";
        }
        ...
        if (s3Badge) {
          ...
          s3Badge.className = isMerger ? "text-xs font-bold px-3 py-1 rounded-lg bg-amber-100 text-amber-800 border border-amber-300" : "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300";
        }
      }
    } else if (route === "sub2") {
      if (kapId === "all") {
        s1Val.textContent = "76.5 (Tinggi)";
        if (s1Badge) { s1Badge.textContent = "Rerata ANBK Literasi"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        s2Val.textContent = "71.9 (Sedang)";
        if (s2Badge) { s2Badge.textContent = "Rerata ANBK Numerasi"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
        s3Val.textContent = "88.5% Unggul";
        if (s3Badge) { s3Badge.textContent = "Akreditasi A/B"; s3Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-purple-100 text-purple-800 border border-purple-300"; }
      } else {
        ...
        if (s1Badge) { s1Badge.textContent = "Literasi Rapor"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
        s2Val.textContent = item ? item.numerasi : "70.0 (Sedang)";
        if (s2Badge) { s2Badge.textContent = "Numerasi Rapor"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        ...
        if (s3Badge) {
          ...
          s3Badge.className = isPrioritas ? "text-xs font-bold px-3 py-1 rounded-lg bg-red-100 text-red-800 border border-red-300" : "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300";
        }
      }
    } else if (route === "sub3") {
      if (kapId === "all") {
        s1Val.textContent = "88.2%";
        if (s1Badge) { s1Badge.textContent = "Ruang Kelas SPM Layak"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        s2Val.textContent = "18 Ruang";
        if (s2Badge) { s2Badge.textContent = "Rusak Berat (DAK 2026)"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-red-100 text-red-800 border border-red-300"; }
        s3Val.textContent = "94.5%";
        if (s3Badge) { s3Badge.textContent = "Lab & Perpus Aktif"; s3Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
      } else {
        ...
        if (s1Badge) { s1Badge.textContent = item ? item.spm : "Layak"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        ...
        if (s2Badge) {
          ...
          s2Badge.className = hasHeavy ? "text-xs font-bold px-3 py-1 rounded-lg bg-red-100 text-red-800 border border-red-300" : "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300";
        }
        ...
        if (s3Badge) {
          ...
          s3Badge.className = isUrgent ? "text-xs font-bold px-3 py-1 rounded-lg bg-amber-100 text-amber-800 border border-amber-300" : "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300";
        }
      }
    } else if (route === "sub4") {
      if (kapId === "all") {
        s1Val.textContent = "7 Usulan Optimal";
        if (s1Badge) { s1Badge.textContent = "MCDM TOPSIS"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        s2Val.textContent = "93.2%";
        if (s2Badge) { s2Badge.textContent = "Rerata Kecocokan"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
        s3Val.textContent = "Hemat 18.5 km";
        if (s3Badge) { s3Badge.textContent = "Efisiensi Jarak Guru"; s3Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-purple-100 text-purple-800 border border-purple-300"; }
      } else {
        ...
        if (s1Badge) { s1Badge.textContent = "Skor MCDM"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        s2Val.textContent = item ? `${item.jarakAsal} -> ${item.jarakBaru}` : "3.5 km";
        if (s2Badge) { s2Badge.textContent = "Pangkas Jarak"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
        s3Val.textContent = item ? item.mapel : "Pemerataan Mapel";
        if (s3Badge) { s3Badge.textContent = "Kebutuhan Guru"; s3Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-purple-100 text-purple-800 border border-purple-300"; }
      }
    } else if (route === "sub5") {
      if (kapId === "all") {
        s1Val.textContent = "486 Lembaga";
        if (s1Badge) { s1Badge.textContent = "100% Izin Operasional"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-emerald-100 text-emerald-800 border border-emerald-300"; }
        s2Val.textContent = "24 Prestasi";
        if (s2Badge) { s2Badge.textContent = "Tingkat DIY & Nasional"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-amber-100 text-amber-800 border border-amber-300"; }
        s3Val.textContent = "12 Inovasi";
        if (s3Badge) { s3Badge.textContent = "Coding & Vokasi"; s3Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-purple-100 text-purple-800 border border-purple-300"; }
      } else {
        ...
        if (s1Badge) { s1Badge.textContent = item ? item.jenjang : "Sekolah"; s1Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-blue-100 text-blue-800 border border-blue-300"; }
        s2Val.textContent = item ? item.prestasi : "Juara Berprestasi";
        if (s2Badge) { s2Badge.textContent = "Penghargaan"; s2Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-amber-100 text-amber-800 border border-amber-300"; }
        s3Val.textContent = item ? item.inovasi : "Inovasi Pembelajaran";
        if (s3Badge) { s3Badge.textContent = "Program Unggulan"; s3Badge.className = "text-xs font-bold px-3 py-1 rounded-lg bg-purple-100 text-purple-800 border border-purple-300"; }
      }
    }
  }
          ...
          <tr class="${isSelected ? 'bg-amber-50/80 border-l-4 border-l-amber-600' : 'hover:bg-slate-50'} transition border-b border-slate-200">
            ...
            <td class="p-3">
              <button onclick="app.toggleKapanewon('${kId}', '${SecurityUtils.escapeHTML(a.kapanewon)}')" class="px-3.5 py-1.5 ${isSelected ? 'bg-slate-900 hover:bg-slate-800 text-amber-400 border border-slate-700' : 'bg-amber-600 hover:bg-amber-700 text-white'} rounded-lg text-xs font-bold whitespace-nowrap shadow-sm transition inline-flex items-center gap-1" aria-label="${isSelected ? 'Batalkan pilihan wilayah ' + SecurityUtils.escapeHTML(a.kapanewon) : 'Pilih wilayah ' + SecurityUtils.escapeHTML(a.kapanewon)}" title="${isSelected ? 'Klik untuk membatalkan filter' : 'Klik untuk memfilter data wilayah ini'}">
                ${isSelected ? 'Batal Pilih <svg width="14" height="14" class="flex-shrink-0 w-3.5 h-3.5 inline-block ml-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg>' : 'Pilih Wilayah <svg width="14" height="14" class="flex-shrink-0 w-3.5 h-3.5 inline-block ml-0.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M14 5l7 7m0 0l-7 7m7-7H3"/></svg>'}
              </button>
            </td>
          </tr>
        `;
      ...
      if (match) {
        ...
        if (key === activeKey) {
          ...
          iconSpan.innerHTML = this.sortDirection === 'asc'
            ? '<svg width="14" height="14" class="flex-shrink-0 w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 19V5M5 12l7-7 7 7"/></svg>'
            : '<svg width="14" height="14" class="flex-shrink-0 w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M12 5v14M19 12l-7 7-7-7"/></svg>';
        } else {
          ...
          iconSpan.innerHTML = '<svg width="14" height="14" class="flex-shrink-0 w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M7 15l5 5 5-5M7 9l5-5 5 5"/></svg>';
        }
      }
    });
  }
  ...
  renderSub4() {
    ...
    tbody.innerHTML = data.map(item => `
      <tr>
        ...
        <td class="p-3 font-semibold text-slate-800 text-xs whitespace-nowrap">${SecurityUtils.escapeHTML(item.kalurahan)}</td>
  ...
  updateUserUI() {
    ...
    if (this.user) {
      desktopContent = `
        <span class="text-xs text-amber-300 font-bold whitespace-nowrap mr-1 inline-flex items-center gap-1.5"><svg width="14" height="14" class="w-3.5 h-3.5 text-amber-300 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> ${SecurityUtils.escapeHTML(this.user.name)}</span>
        <a href="${relPrefix}security/index.html" class="px-2.5 py-1.5 bg-blue-900 hover:bg-blue-800 text-white border border-blue-700 rounded-lg font-bold text-xs transition inline-flex items-center gap-1.5 shadow-sm"><svg width="14" height="14" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> Audit Log</a>
      ...
      mobileContent = `
        <div class="w-full flex items-center justify-between bg-slate-800 p-2.5 rounded-lg border border-slate-700 mb-2">
          <span class="text-xs text-amber-300 font-bold"><svg width="14" height="14" class="flex-shrink-0 w-3.5 h-3.5 text-amber-300 inline-block mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> ${SecurityUtils.escapeHTML(this.user.name)}</span>
        ...
        <a href="${relPrefix}security/index.html" class="w-full text-center px-3 py-2 bg-blue-900 hover:bg-blue-800 text-white rounded-lg font-bold text-xs transition block border border-blue-700"><svg width="14" height="14" class="w-3.5 h-3.5 inline-block mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> Audit Log Keamanan</a>
      `;
    } else {
      desktopContent = `
        <a href="${relPrefix}security/index.html" class="px-3 py-1.5 bg-blue-900/90 hover:bg-blue-800 text-white border border-blue-600/80 rounded-lg font-bold text-xs transition inline-flex items-center gap-1.5 shadow-sm"><svg width="14" height="14" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> Status Keamanan</a>
        <a href="${relPrefix}login/index.html" class="px-3.5 py-1.5 bg-amber-600 hover:bg-amber-700 text-white font-extrabold rounded-lg text-xs transition shadow-sm inline-flex items-center gap-1.5"><svg width="14" height="14" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg> Masuk Pimpinan</a>
      `;
      mobileContent = `
        <div class="grid grid-cols-2 gap-2 w-full">
          <a href="${relPrefix}security/index.html" class="px-2 py-2 bg-blue-900 hover:bg-blue-800 text-white border border-blue-700 rounded-lg font-bold text-xs transition inline-flex items-center justify-center gap-1.5"><svg width="14" height="14" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> <span>Status Keamanan</span></a>
          <a href="${relPrefix}login/index.html" class="px-2 py-2 bg-amber-600 hover:bg-amber-700 text-white font-extrabold rounded-lg text-xs transition inline-flex items-center justify-center gap-1.5 shadow-sm"><svg width="14" height="14" class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg> <span>Masuk Pimpinan</span></a>
        </div>
      `;
    }
    ...
    const modalHtml = `
      <div id="auditModalOverlay" class="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white border border-slate-300 rounded-2xl max-w-3xl w-full p-6 shadow-2xl space-y-4 max-h-[85vh] flex flex-col">
          <div class="flex items-center justify-between border-b border-slate-200 pb-3">
            <div>
              <h3 class="text-sm font-black text-slate-900 uppercase tracking-wide"><svg width="14" height="14" class="w-3.5 h-3.5 inline-block mr-1 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg> LOG AUDIT KEAMANAN &amp; INTEGRITAS DATA</h3>
